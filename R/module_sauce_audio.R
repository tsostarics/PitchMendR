...
playAudio_sauceServer <- function(id, loadedFile, currentWave, destroyLoadedAudio,
                            audioInfo, nPlotted, plotSubset, plot_brush, fileHandler) {
  moduleServer(id, function(input, output, session) {
    ...
    playAudio <- reactive({
      ...
      if (is.null(loadedFile$data) || !nPlotted$is_one || is.null(audioInfo$audioDirectory) )
        return(NULL)
      ...
      file_to_open <- file.path(audioInfo$audioDirectory(),
                                gsub(".Pitch$", ".wav", fileHandler$filenames[fileHandler$isPlotted]))
